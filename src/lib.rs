use wavers::{read, write, Samples, WaversError};

pub fn repeat_audio(min_duration_seconds: f64, input_file_name: &str) -> Result<(), WaversError> {
    let file_extension = input_file_name.split('.').last().unwrap();
    let file_extension = format!(".{}", file_extension);

    let (samples, sample_rate): (Samples<i16>, i32) = read::<i16, _>(input_file_name)?;
    ...
    if n_repeats > 1 {
        ...
        write(output_file_name.clone(), &out_samples, sample_rate, 1)?;
